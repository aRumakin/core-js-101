...
function getRectangleString(width, height) {
  ...
  return firstLine + arr.join('') + lastLine;
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (str[i].match(/[a-z]/)) {
      res += str[i].charCodeAt() >= 110
        ? String.fromCharCode(str[i].charCodeAt() - 13)
        : String.fromCharCode(str[i].charCodeAt() + 13);
    } else if (str[i].match(/[A-Z]/)) {
      res += str[i].charCodeAt() >= 78
        ? String.fromCharCode(str[i].charCodeAt() - 13)
        : String.fromCharCode(str[i].charCodeAt() + 13);
    } else {
      res += str[i];
...
function getCardId(value) {
  ...
  const keyCurr = value.slice(-1);
  const valCurr = value.replace(keyCurr, '');
  return 13 * key.indexOf(keyCurr) + val.indexOf(valCurr);
}
